...
-- Default Admin User for TaksoRide
-- Email: admin@taksoride.com | Password: admin123
INSERT IGNORE INTO `cab_tbl_users` (`user_id`, `pwd_raw`, `password_hash`, `address`, `email`, `firstname`, `lastname`, `sex`, `phone`, `country`, `referal_code`, `account_type`, `route_id`, `last_login_date`, `login_count`, `referral_count`, `referral_discounts_count`, `account_create_date`, `push_notification_token`, `is_activated`, `user_rating`, `account_active`, `photo_file`, `wallet_amount`, `country_code`, `country_dial_code`) VALUES (1, 'admin123', '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', 'TaksoRide HQ', 'admin@taksoride.com', 'Admin', 'TaksoRide', 'male', '+1234567890', 'Global', 'TAKSO01', '3', '1', NOW(), '0', '0', '0', NOW(), '', '1', '5', '1', '0', '0', 'us', '+1');
...
CREATE TABLE IF NOT EXISTS `cab_tbl_drivers` (
  ...
  `allow_alt_cars` TINYINT(1) NOT NULL DEFAULT 1 COMMENT '0 = Driver will not receive trips from alternate vehicles. 1 = Driver has chosen to receive trips from alternate vehicles',
...
INSERT IGNORE INTO `cab_tbl_franchise` (`id`,`franchise_name`,`franchise_desc`, `franchise_commision`) VALUES (1,'TaksoRide','Default franchise (Company)','100');
...
CREATE TABLE IF NOT EXISTS `cab_tbl_bookings` (
  ...
  `bk_alt_cars` VARCHAR(50) NULL COMMENT 'Alternate car categories that can receive trip requests for this car type if it is unavailable for this booking',  
  `ride_share` TINYINT(1) UNSIGNED DEFAULT 0 COMMENT '0-do not ride share this booking, 1 = ride share this booking',
  ...
  `scheduled_driver` INTEGER UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Driver who have accepted the scheduled booking',
  `retry_booking` TINYINT(1) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'Number of times the rider retried booking after not finding a driver',
...
CREATE TABLE IF NOT EXISTS `cab_tbl_rides_tariffs` (
  ...
  `alt_cars` VARCHAR(50) NULL COMMENT 'Alternate car categories that can receive trip requests for this car type if it is unavailable',  
...
-- TaksoRide Service Configuration Table for API Keys
CREATE TABLE IF NOT EXISTS `cab_tbl_service_config` (
  `id` INTEGER UNSIGNED NOT NULL AUTO_INCREMENT,
  `service_name` VARCHAR(100) NOT NULL COMMENT 'Name of the service (e.g., Google Maps, Firebase, PubNub)',
  `service_key` VARCHAR(100) NOT NULL COMMENT 'Unique identifier for the service',
  `config_data` TEXT NULL COMMENT 'JSON encoded configuration data including API keys',
  `is_active` TINYINT(1) UNSIGNED DEFAULT '1' COMMENT 'Is this service active',
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COLLATE=utf8_unicode_ci COMMENT='Service configuration and API keys';

-- Insert default service configurations
INSERT IGNORE INTO `cab_tbl_service_config` (`service_key`, `service_name`, `description`, `config_data`, `is_active`) VALUES 
('google_maps', 'Google Maps API', 'Google Maps API for geocoding, directions, and places autocomplete', '{"api_key":"","enabled":true}', 1),
('firebase', 'Firebase Cloud Messaging', 'Firebase for push notifications to mobile apps', '{"server_key":"","sender_id":"","project_id":"","credentials_json":""}', 1),
('pubnub', 'PubNub Real-time', 'PubNub for real-time messaging and location tracking', '{"publish_key":"","subscribe_key":"","secret_key":""}', 1),
('twilio', 'Twilio SMS', 'Twilio for SMS notifications and OTP', '{"account_sid":"","auth_token":"","phone_number":""}', 0),
('stripe', 'Stripe Payment', 'Stripe payment gateway integration', '{"secret_key":"","publishable_key":"","webhook_secret":""}', 0),
('paypal', 'PayPal Payment', 'PayPal payment gateway integration', '{"client_id":"","client_secret":"","mode":"sandbox"}', 0),
('paystack', 'Paystack Payment', 'Paystack payment gateway for African markets', '{"secret_key":"","public_key":""}', 0),
('flutterwave', 'Flutterwave Payment', 'Flutterwave payment gateway', '{"secret_key":"","public_key":"","encryption_key":""}', 0),
('smtp_email', 'SMTP Email', 'SMTP configuration for sending emails', '{"host":"","port":"587","username":"","password":"","from_email":"","from_name":"TaksoRide"}', 1),
('redis', 'Redis Cache', 'Redis configuration for caching and real-time data', '{"host":"127.0.0.1","port":"6379","password":"","database":"0"}', 1),
('app_settings', 'App Settings', 'General application settings', '{"app_name":"TaksoRide","app_tagline":"Your Reliable Ride Partner","currency":"USD","currency_symbol":"$","timezone":"UTC"}', 1);
