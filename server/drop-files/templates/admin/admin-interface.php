...
<head>
  ...
  // Try from settings array
  elseif(isset($settings_data2['google-maps-api-key']) && !empty($settings_data2['google-maps-api-key'])) {
      $gmaps_key = $settings_data2['google-maps-api-key'];
  }
  // Load from database as fallback
  elseif(isset($GLOBALS['DB'])) {
      $query = sprintf('SELECT option_value FROM %stbl_settings WHERE option_name = "google-maps-api-key" LIMIT 1', DB_TBL_PREFIX);
      $result = @mysqli_query($GLOBALS['DB'], $query);
      if($result) {
          if($row = mysqli_fetch_assoc($result)) {
              $gmaps_key = $row['option_value'];
          }
  ...
  if(!empty($gmaps_key)):
  ?>
  <script async defer src="https://maps.googleapis.com/maps/api/js?key=<?php echo htmlspecialchars($gmaps_key); ?>&libraries=places,drawing,geometry&callback=initMap"></script>
  <script>
  ...
  window.gm_authFailure = function() {
      // Authentication failed - silently handle
  };
  </script>
  <?php else: ?>
  <script>
  // Google Maps not configured - create dummy function to prevent errors
  function initMap() {}
  window.gm_authFailure = function() {};
  </script>
  <?php endif; ?>
  ...
  /* When sidebar is collapsed, show mini logo */
...
<div class="wrapper">
  ...
  <header class="main-header">
    ...
    <a href="<?php echo SITE_URL; ?>" class="logo">
      ...
      <span class="logo-mini"><img src='../img/company-logo-small.png?v=<?php echo time(); ?>' alt="Logo" /></span>
      <!-- Full logo for expanded sidebar -->
      <span class="logo-lg"><img src='../img/company-logo-small.png?v=<?php echo time(); ?>' alt="Logo" /></span>
    </a>
    ...
    <section class="sidebar">
     ...
     <hr /> -->
      ...
      ?> 
        ...
        ?> 
          <li class="<?php if(isset($GLOBALS['admin_template']['active_menu']) && $GLOBALS['admin_template']['active_menu'] == "alltransactions"){echo "active" ;} ?>"><a href="all-transactions.php"><i class="fa fa-usd"></i> <span>Transactions</span></a></li>
        <?php 
          }      
        ?>
        ...
        <?php 
          if($_SESSION['account_type'] == 3){       
        ?> 
          <li class="<?php if(isset($GLOBALS['admin_template']['active_menu']) && $GLOBALS['admin_template']['active_menu'] == "subscriptions"){echo "active" ;} ?>"><a href="subscription-plans.php"><i class="fa fa-credit-card"></i> <span>Subscription Plans</span></a></li>
          <li class="<?php if(isset($GLOBALS['admin_template']['active_menu']) && $GLOBALS['admin_template']['active_menu'] == "cashback"){echo "active" ;} ?>"><a href="cashback-manager.php"><i class="fa fa-gift"></i> <span>Cashback Manager</span></a></li>
