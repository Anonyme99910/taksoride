...
if(isset($_POST['schedule-bk'])){
    if(strtotime($_POST['date'] . " " . $_POST['time-hour'] . ":" .$_POST['time-min']) < (time() + 600)){
...
if($_POST['zonetypeoption'] == "city"){
    ...
    $datetime = !empty($scheduled) ? gmdate('Y-m-d H:i:s',strtotime($_POST['date'] . " " . $_POST['time-hour'] . ":" . $_POST['time-min'])) : gmdate('Y-m-d H:i:s');
...
$query = sprintf('INSERT INTO %stbl_bookings (waypoint1_address,waypoint1_long,waypoint1_lat,waypoint2_address,waypoint2_long,waypoint2_lat,dispatch_mode,cur_symbol,cur_exchng_rate,cur_code,completion_code,scheduled,user_firstname,user_lastname,user_phone,user_id, pickup_datetime, pickup_address, pickup_long, pickup_lat, dropoff_address, dropoff_long,dropoff_lat,estimated_cost,route_id,ride_id,payment_type,date_created) VALUES'.
'("%s","%s","%s","%s","%s","%s","%d","%s","%s","%s","%s","%d","%s","%s","%s","%d","%s","%s","%s","%s","%s","%s","%s","%s","%d","%d","%d","%s")', 
DB_TBL_PREFIX,
$waypoint1_address,
$waypoint1_lat,
$waypoint1_lng,
