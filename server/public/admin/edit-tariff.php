    ...
    if(! $result = mysqli_query($GLOBALS['DB'], $query)){
        ...
        foreach($_POST['car_type'] as $cartype_id){
            ...
            foreach($rides_array as $ride){
                if(isset($_POST["altv-{$cartype_id}-{$ride['id']}"])){
                    $alt_cars .= $ride['id'] . ",";
                }
            }
            ...
            $DB_values_string .= "({$cartype_id},%2\$d,{$cc},{$cpkr},{$cpmr},{$pcr},{$dcr},{$ncc},{$ncpkr},{$ncpmr},{$npcr},{$ndcr},{$pp_enabled},{$pp_start_time},{$pp_end_time},{$pp_active_days},{$pp_charge_type},{$pp_charge},{$fare_compute},{$ride_share},{$ind},{$nind},\"{$alt_cars}\"),";
