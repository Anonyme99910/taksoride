...
function sbookingbroadcast(){
    ...
    $query = sprintf('SELECT %1$stbl_bookings.*,%1$stbl_users.push_notification_token AS push_token,%1$stbl_bookings.user_id AS customer_id,%1$stbl_users.disp_lang AS u_lang,%1$stbl_users.photo_file,%1$stbl_users.user_rating FROM %1$stbl_bookings 
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            ...
            $res = array("error"=>"An error has occured");
    ...
    if($booking_data['status'] != 0){ //booking not pending. its either cancelled or serviced
        ...
        $res = array("error"=>"Booking not pending. its either cancelled or serviced");
        echo json_encode($res); 
        exit;

    }elseif(!empty($booking_data['scheduled_driver']) || !empty($booking_data['driver_id'])){ //already accepted by another driver

        $res = array("error"=>"Booking already accepted by another driver");
        echo json_encode($res); 
        exit;
    ...
    //check if scheduled booking data is already on redis

    $sbooking_redis_data = $redis->get("sbkdata:{$booking_id}"); //get scheduled booking data from redis

    if(!empty($sbooking_redis_data)){ //booking was previously broadcasted

        $res = array("error"=>"Drivers have already been notified of this booking");
        echo json_encode($res); 
        exit;        
    }


    //not previously broadcasted. create an entry in redis. This will show up in the driver app scheduled trips page
    $photo_file = isset($booking_data['photo_file']) ? $booking_data['photo_file'] : "0";
    $photo_file = SITE_URL . "ajaxuserphotofile.php?file=" . $photo_file;
    $sdata = ["firstname" => $booking_data['user_firstname'],"phone" => $booking_data['user_phone'],"photo" => $photo_file,"bk_id" => $booking_id, "p_loc" => $booking_data['pickup_address'], "d_loc" => $booking_data['dropoff_address'], "p_lat" => $booking_data['pickup_lat'], "p_lng" => $booking_data['pickup_long'],"d_lat" => $booking_data['dropoff_lat'],"d_lng" => $booking_data['dropoff_long'], "p_time" => $booking_data['pickup_datetime'], "bk_time" => $booking_data['date_created'],"ride_id" => $booking_data['ride_id'],"route_id" => $booking_data['route_id'],"fare" => $booking_data['estimated_cost'],"driver_id" => 0,"user_rating" => $booking_data['user_rating'],"waypoint1" => "","waypoint2" => ""];
    $redis->zAdd("sbknotifs:{$booking_data['route_id']}",strtotime($booking_data['pickup_datetime'] . " UTC"),"sbkdata:{$booking_id}");
    $bk_data_ttl =  abs(strtotime($booking_data['pickup_datetime'] . " UTC") - time()) + 86400;
    ...
    sendPushNotification(__("New scheduled booking"), __("A customer has scheduled a new booking"),"/topics/driver-route-{$booking_data['route_id']}",null,json_encode($ext_msg_data),null,null);
...
function sbookingassigndriver(){
    ...
    if($result = mysqli_query($GLOBALS['DB'], $query)){
        if(mysqli_num_rows($result)){
            ...
            if($driver_data['available'] && strtotime($driver_data['location_date'] . ' UTC') < (time() - LOCATION_INFO_VALID_AGE)){
                return array("error"=>"Driver is offline.");
            }elseif(empty($driver_data['available'])){
                return array("error"=>"Driver is offline.");
            }
        }else{

            return array("error"=>"Invalid driver record.");
        }
    }else{
        return array("error"=>"Error occured trying to read driver data from database.");
    }
    

    if($booking_data['status'] != 0){ //booking not pending. its either cancelled or serviced

        return array("error"=>"Booking not pending. its either cancelled or serviced");

    }elseif(!empty($booking_data['scheduled_driver']) && $booking_data['scheduled_driver'] != $driver_id){ //already accepted by another driver

        return array("error"=>"Booking already accepted by another driver");

    }elseif($booking_data['ride_id'] != $driver_data['ride_id']){

        return array("error"=>"Driver vehicle category does not match");
    ...
    if($file_bk_loc){
        ...
        if ($file_loc === false ) {
            return array("error"=>"Another driver has accepted the booking");    
        }
    ...
    if(!empty($sbooking_redis_data)){ //booking was previously broadcasted
        ...
        if(empty($sb_redis_data)){ //booking data not available
            ...
            return array("error"=>"Booking data not found on Redis");
        }

        if(!empty($sb_redis_data['driver_id'])){
            if($file_bk_loc){
                fclose($file_bk_loc); //release locked file
            }
            //driver already been assigned
            return array("error"=>"Another driver has accepted the booking"); 
        }

        //add driver to the booking as the assigned driver
        $sb_redis_data['driver_id'] = $driver_id;
    ...
    }else{ //not previously broadcasted. create an entry in redis. This will show up in the driver app scheduled trips page
        $photo_file = isset($booking_data['photo_file']) ? $booking_data['photo_file'] : "0";
        $photo_file = SITE_URL . "ajaxuserphotofile.php?file=" . $photo_file;
        $sdata = ["firstname" => $booking_data['user_firstname'],"phone" => $booking_data['user_phone'],"photo" => $photo_file,"bk_id" => $booking_id, "p_loc" => $booking_data['pickup_address'], "d_loc" => $booking_data['dropoff_address'], "p_lat" => $booking_data['pickup_lat'], "p_lng" => $booking_data['pickup_long'],"d_lat" => $booking_data['dropoff_lat'],"d_lng" => $booking_data['dropoff_long'], "p_time" => $booking_data['pickup_datetime'], "bk_time" => $booking_data['date_created'],"ride_id" => $booking_data['ride_id'],"route_id" => $booking_data['route_id'],"fare" => $booking_data['estimated_cost'],"driver_id" => $driver_id,"user_rating" => $booking_data['user_rating'],"waypoint1" => "","waypoint2" => ""];
        $redis->zAdd("sbknotifs:{$booking_data['route_id']}",strtotime($booking_data['pickup_datetime'] . " UTC"),"sbkdata:{$booking_id}");
        $bk_data_ttl =  abs(strtotime($booking_data['pickup_datetime'] . " UTC") - time()) + 86400;
        $redis->setEx("sbkdata:{$booking_id}", $bk_data_ttl ,serialize($sdata)); //store booking data
    }
    ...
    //update database with assigned driver
    ...
    if(!empty($booking_data['push_token'])){
        $ext_msg_data = array(
            'mode' => 1,
    ...
    if(!empty($driver_data['push_notification_token'])){
        $ext_msg_data = array(
            'mode' => 1,
            'title' => "New scheduled booking",
            'message' => preg_replace("/\"/", '&#039;', htmlspecialchars("You have been assigned to a customer scheduled booking")),
            'action' => 'scheduled_bk'
        );        
        
        
        sendPushNotification(__("New scheduled booking"), __("You have been assigned to a customer scheduled booking"),$driver_data['push_notification_token'],null,json_encode($ext_msg_data),null,null);
    }


    return array("success"=>"The driver has been assigned to the scheduled booking","status" => 1);
...
function bookingassigndriver(){
    ...
    $booking_id = (int) $_POST['booking_id'];
    $driver_id = (int) $_POST['driver_id'];
    $scheduled_booking_status = (int) $_POST['schd_bk'];
    $driver_data = [];
    ...
    if($scheduled_booking_status == 2){ //driver assignment was from scheduled booking page. handle it
        $res = sbookingassigndriver();
        if(is_array($res)){
        ...
        $resp = array("error"=>"An error has occured");
...
function submitcontactform(){
    ...
    if(EMAIL_TRANSPORT == 1){
        ...
        sendMail(CONTACT_EMAIL, 'Contact Form Message  - '.$subject, $message);
    }
